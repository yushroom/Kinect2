	...
	)
{
	...
	const UINT nBufferSize = NUI_DEPTH_RAW_HEIGHT * NUI_DEPTH_RAW_WIDTH;
	static UINT16 pDepthBuffer[nBufferSize];
	UINT16 *pBuffer = NULL;
	if (m_pDepthFilePtr) {
		
		if (nBufferSize != fread_s(pDepthBuffer, sizeof(UINT16)*nBufferSize, sizeof(UINT16), nBufferSize, m_pDepthFilePtr)) {
			is_eof = true;
			return E_INVALIDARG;
		}
	}
	//fread(pBuffer, sizeof(UINT16), nBufferSize, m_pDepthFilePtr);
	else {
		UINT nBufferSize = 0;
...
#ifdef UNDISTORT
		*pDepth = id < bufferLength? (m_pDepthFilePtr ? pDepthBuffer[i] : pBuffer[i]) : 0;
#else
		*pDepth = m_pDepthFilePtr ? pDepthBuffer[i] : pBuffer[i];
#endif
		*pRawDepth = m_pDepthFilePtr ? pDepthBuffer[i] : pBuffer[i];
	}
...
KinectFusionProcessor::KinectFusionProcessor() :
	...
	m_pCameraPosePtr(nullptr),
	m_pDepthFilePtr(nullptr)
{
...
//#define PRE "1to2"
//#endif
//#define DEPTH_PATH FOLDER PRE "-620.bin"
//	fopen_s(&m_pDepthFilePtr, global_depth_path.c_str(), "rb");
//	//if (!m_pDepthFilePtr) exit(0);
//#endif
//
////#define READ_CAMERAPOSE_FROM_FILE
//#ifdef READ_CAMERAPOSE_FROM_FILE
//	fopen_s(&m_pCameraPosePtr, "D:\\cvpr\\corner\\camerapose.txt", "r");
//
//	/*
//	   data: [ 9.9989441466012186e-001, -2.2767468255576358e-003,
//       -1.4351862436095226e-002, 2.1977706472568696e-003,
//       9.9998237038136273e-001, -5.5162243113602138e-003,
//       1.4364168464423691e-002, 5.4840997769459460e-003,
//       9.9988179067025829e-001 ]
//	T: !!opencv-matrix
//	   rows: 3
//	   cols: 1
//	   dt: d
//	   data: [ -1.9443198716845778e+000, -4.9122768096460678e+000,
//		   -9.2172052904982227e-001 ]
//	*/
//	float rotate[3][3] = { 
//			{ 9.9989441466012186e-001, -2.2767468255576358e-003, -1.4351862436095226e-002 }, 
//			{ 2.1977706472568696e-003,
//			9.9998237038136273e-001, -5.5162243113602138e-003 },
//			{ 1.4364168464423691e-002, 5.4840997769459460e-003,
//			9.9988179067025829e-001 } };
//	float tranlate[3] = { -1.9443198716845778e+000, -4.9122768096460678e+000, -9.2172052904982227e-001 };
//	codex::math::matrix::identity(m_trans_v1_to_v2);
//	for (int i = 0; i < 3; i++)
//		for (int j = 0; j < 3; j++)
//			m_trans_v1_to_v2.n[i][j] = rotate[i][j];
//	for (int i = 0; i < 3; i++)
//		m_trans_v1_to_v2.n[i][3] = tranlate[i];
//#endif
}
...
void KinectFusionProcessor::setDepthFilePath(const char *depth_file_path)
{
	fopen_s(&m_pDepthFilePtr, depth_file_path, "rb");
}
...
HRESULT KinectFusionProcessor::InitializeDefaultSensor()
{
	...
	if (m_pDepthFilePtr) {
		SetStatusMessage(L"Read from file");
		m_bKinectFusionInitialized = true;
	}
	else if (!m_pNuiSensor || FAILED(hr))
	{
...
/// </summary>
/// <returns>S_OK on success, otherwise failure code</returns>
HRESULT KinectFusionProcessor::GetKinectFrames(bool &colorSynchronized, bool& is_eof)
{
	HRESULT hr = S_OK;
	...
	hr = CopyDepth(pDepthFrame, is_eof);
...
inline Matrix4 read_camera_pose(FILE* fp) {
	Matrix4 mat;
	for (int i = 0; i < 16; ++i)
		fscanf_s(fp, "%g", &(&mat.M11)[i]);	 
	return mat;
}

inline Matrix4 transform(const Matrix4& mat, matrix4x4f &trans) {
	matrix4x4f m;
	for (int i = 0; i < 16; ++i)
		m.m[i] = (&mat.M11)[i];
	m *= trans;
...
bool KinectFusionProcessor::ProcessDepth()
{
	...
	bool is_eof;
	hr = GetKinectFrames(colorSynchronized, is_eof);
	...
	else
	{
		...
		calculatedCameraPose = read_camera_pose(m_pCameraPosePtr);	
		m_worldToCameraTransform = Transpose(calculatedCameraPose);
		//m_worldToCameraTransform = Transpose(transform(calculatedCameraPose, m_trans_v1_to_v2));
