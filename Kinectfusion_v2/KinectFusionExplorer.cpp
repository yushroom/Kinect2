...
inline std::vector<std::string> get_argv() {
	int w_argc = 0;
...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE, _In_ LPWSTR, _In_ int nCmdShow)
{
	...
	std::string depth_file_path = argv.size() > 1 ? argv[1] : std::string();
	CKinectFusionExplorer application(depth_file_path);
...
/// Constructor
/// </summary>
CKinectFusionExplorer::CKinectFusionExplorer(std::string depth_file_path) :
	m_hWnd(nullptr),
    ...
    m_bUIUpdated(false)
{
	if (depth_file_path != std::string()) m_processor.setDepthFilePath(depth_file_path.c_str());
}
...
void CKinectFusionExplorer::InitializeUIControls()
{
    ...
    case 128:
        ...
        break;
    default:
        m_params.m_reconstructionParams.voxelCountX = 512;	// set to medium default
        CheckDlgButton(m_hWnd, IDC_VOXELS_X_512, BST_CHECKED);
        break;
    }
